...
namespace Phantasma.Spook.Swaps
{
    public class TokenSwapper
    {
        ...
        public TokenSwapper(KeyPair swapKey)
        {
            Console.WriteLine("Initializing Phantasma swap server...");
            ...
            {
                Console.WriteLine("Interop file missing...");
                return;
            }
            ...
            foreach (var entry in interopBlocks)
            {
                ...
                if (interop != null)
                {
                    ...
                    //Console.WriteLine($"Listening for {entry.Key} swaps at address {interop.LocalAddress}, starting from block {blockHeight}...");
                }
            }
        ...
        private void ProcessSwaps(ChainInterop interop, IEnumerable<ChainSwap> swaps)
        {
            ...
            foreach (var temp in swaps)
            {
                ...
                {
                    Console.WriteLine($"Executing {swap.sourceChain} swap: {swap.sourceAddress} sent {swap.amount} {swap.symbol} to {swap.destinationAddress}");
