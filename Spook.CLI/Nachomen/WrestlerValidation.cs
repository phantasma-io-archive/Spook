...
namespace Phantasma.Spook.Nachomen
{
    public static class WrestlerValidation
    {
        ...
        private static bool HasValidMoveset(NachoWrestler nachoWrestler)
        {
            if (Rules.GetSecondaryMoveFromGenes(nachoWrestler.genes) == WrestlingMove.Pray) return false;
            if (Rules.GetSecondaryMoveFromGenes(nachoWrestler.genes) == WrestlingMove.Hyper_Slam) return false;
            return true;
        }
        ...
        private static bool HasValidVisual(NachoWrestler nachoWrestler)
        {
            var legs = Luchador.GetBodyPart(BodyPart.Legs).Variation;
            if (legs > 7) return false;
        ...
        public static bool IsValidWrestler(NachoWrestler nachoWrestler)
        {
            return HasValidMoveset(nachoWrestler) && HasValidVisual(nachoWrestler);
        }
