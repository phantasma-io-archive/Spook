...
namespace Phantasma.Spook
{
    ...
    public class CLI
    {
        ...
        {
            if (_settings.Node.StorageConversion)
            {
                Func<string, IKeyValueStoreAdapter> fileStorageFactory  = (name)
                    => new BasicDiskStore(_settings.Node.StoragePath + name + ".csv");

                Func<string, IKeyValueStoreAdapter> dbStorageFactory    = (name)
                    => new DBPartition(logger, _settings.Node.DbStoragePath+ name);

                Func<string, IKeyValueStoreAdapter> verificationStorageFactory  = (name)
                    => new BasicDiskStore(_settings.Node.VerifyStoragePath + name + ".csv");
                ...
                if (_settings.Node.RandomSwapData)
                {
